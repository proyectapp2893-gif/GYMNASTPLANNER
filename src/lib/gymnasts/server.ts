...
export async function getExerciseBank() {
  const { supabase, clubId, error } = await getAuthenticatedClub()
  if (error || !clubId) notFound()
  const { data, error: queryError } = await supabase.from('ejercicios').select('id,nombre,categoria,aparato,descripcion_corta,rangos_repeticiones').order('nombre').limit(300)
  if (queryError) throw new Error(queryError.message)
